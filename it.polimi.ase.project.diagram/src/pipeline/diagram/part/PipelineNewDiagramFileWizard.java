...
public class PipelineNewDiagramFileWizard extends Wizard {
	...
	* @generated
	*/
	public PipelineNewDiagramFileWizard(URI domainModelURI, EObject diagramRoot,
			TransactionalEditingDomain editingDomain) {
		assert domainModelURI != null : "Domain model uri must be specified"; //$NON-NLS-1$
		assert diagramRoot != null : "Doagram root element must be specified"; //$NON-NLS-1$
		assert editingDomain != null : "Editing domain must be specified"; //$NON-NLS-1$

		myFileCreationPage = new WizardNewFileCreationPage(Messages.PipelineNewDiagramFileWizard_CreationPageName,
		...
		myFileCreationPage
				.setFileName(PipelineDiagramEditorUtil.getUniqueFileName(filePath, fileName, "pipeline_diagram")); //$NON-NLS-1$

		diagramRootElementSelectionPage = new DiagramRootElementSelectionPage(
				Messages.PipelineNewDiagramFileWizard_RootSelectionPageName);
		diagramRootElementSelectionPage.setTitle(Messages.PipelineNewDiagramFileWizard_RootSelectionPageTitle);
	...
	*/
	public boolean performFinish() {
		...
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(myEditingDomain,
				Messages.PipelineNewDiagramFileWizard_InitDiagramCommand, affectedFiles) {
		...
		try {
			...
			PipelineDiagramEditorPlugin.getInstance().logError("Save operation failed for: " + diagramModelURI, ex); //$NON-NLS-1$
		} catch (PartInitException ex) {
			PipelineDiagramEditorPlugin.getInstance().logError("Unable to open editor", ex); //$NON-NLS-1$
		}
		return true;
	}
